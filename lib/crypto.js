...
function syncClaudeSettingsAllowedTools() {
  ...
  // Build the full set of server names Delt knows about (connected or not)
  const deltManagedPatterns = new Set();
  for (const integration of _integrationsRegistry.integrations) {
    for (const serverName of Object.keys(integration.mcpServers || {})) {
      deltManagedPatterns.add(`mcp__${serverName}__*`);
    }
  }

  // Merge: keep user's non-Delt entries, replace only Delt-managed ones
  const existing = Array.isArray(settings.allowedTools) ? settings.allowedTools : [];
  const kept = existing.filter(t => !deltManagedPatterns.has(t));
  settings.allowedTools = [...kept, ...mcpPatterns];
